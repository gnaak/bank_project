...
def save_deposit_products(request):
    ...
    for li in response.get("result").get("baseList"):
        ...
        serializers = SavingProductsSerializer(data=save_data)
        try:
    ...
    for data in options:
        ...
        save_data = {
            ...
            'product' : SavingProducts.objects.filter(fin_prdt_cd=data.get('fin_prdt_cd'))[0].pk,
...
def deposit_products_options(request, fin_prdt_cd):
    ...
    options = response.get("result").get("optionList")
    for a in range(39):
        product = SavingProducts.objects.filter(fin_prdt_cd=fin_prdt_cd)[a]
        ppk = SavingProducts.objects.get(pk=product.pk)
        for li in options:
...
def top_rate(request):
    ...
    products = SavingProducts.objects.get(pk =top_product.product.pk)

    serializer = SavingProductsSerializer(products)
    serializer2 = SavingOptionSerializer(top_product)
